...
public final class DataTypeSerializationUtil {
    ...
    private static final class DataTypeDeserializer extends JsonDeserializer<DataType> {
        ...
        @Override
        public DataType deserialize(final JsonParser p, final DeserializationContext ctxt)
            throws IOException, JacksonException {
            final var node = (JsonNode)p.getCodec().readTree(p);
            final var id = node.asText();
            return DataTypeRegistry.getInstance().availableDataTypes().stream()
                .filter(dt -> dt.getIdentifier().equals(id)).findFirst()
                .orElseThrow(() -> new IOException("Unknown DataType: " + id));
        }
