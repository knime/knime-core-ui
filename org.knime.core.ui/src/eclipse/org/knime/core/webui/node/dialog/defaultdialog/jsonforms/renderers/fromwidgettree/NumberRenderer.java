...
class NumberRenderer extends WidgetTreeControlRendererSpec implements NumberRendererSpec {
    ...
    private Optional<NumberInputWidget> m_annotation;
    ...
    @Override
    public NumberRendererOptions getOptions() {
        if (m_annotation.isEmpty()) {
            return null;
        }
        final var annotation = m_annotation.get();
        return new NumberRendererOptions() {
            ...
            public Optional<NumberInputWidgetValidation[]> getValidations() {
                return Optional.of(Arrays.stream(annotation.validation()).map(InstantiationUtil::createInstance)
                    .toArray(NumberInputWidgetValidation[]::new)).filter(val -> val.length > 0);
            }
