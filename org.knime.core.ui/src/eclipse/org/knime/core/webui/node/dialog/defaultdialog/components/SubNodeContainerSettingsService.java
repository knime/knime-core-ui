...
public final class SubNodeContainerSettingsService implements NodeSettingsService {
    ...
    public String fromNodeSettings(final Map<SettingsType, NodeAndVariableSettingsRO> settings,
        ...
        if (JobManagerSubNodeSettingsUtil.isStreamingExtensionInstalled()) {
            final var renderersAndUiSchemas = JobManagerSubNodeSettingsUtil
                .getJobManagerSubNodeSettingsAndSetSelectedValues(data,
                settings.get(JOB_MANAGER));
            JobManagerSubNodeSettingsUtil.setPersistSchema(persistSchemaMap);

            for (var rendererAndUiSchema : renderersAndUiSchemas) {
                m_renderers.add(rendererAndUiSchema.renderer());
                uiSchemaElements.addObject().setAll(rendererAndUiSchema.uiSchema());
    ...
    public void toNodeSettings(final String jsonSettings, //
        ...
        final var extractedJobManagerSettings = JobManagerSubNodeSettingsUtil.toNodeSettings(dataJson);
