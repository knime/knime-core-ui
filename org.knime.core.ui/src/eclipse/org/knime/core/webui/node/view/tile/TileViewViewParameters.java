...
public final class TileViewViewParameters implements NodeParameters {

    TileViewViewParameters() {
        this((DataTableSpec)null);
    }

    TileViewViewParameters(final NodeParametersInput context) {
        this(context.getInTableSpecs()[0]);
    }

    TileViewViewParameters(final DataTableSpec spec) {
    ...
    @Widget(title = "Show only selected rows",
        description = "When checked, only the selected rows are shown in the tile view.")
    @Layout(InteractivitySection.class)
    ...
    private static final class TitleColumnChoicesProvider extends CompatibleColumnsProvider {
        public TitleColumnChoicesProvider() {
            super(List.of(NominalValue.class, DoubleValue.class));
        }
    }

    private static final class TilesPerRowReference implements ParameterReference<Integer> {
    }

    private static final class PageSizeMinValidation
        implements StateProvider<NumberInputWidgetValidation.MinValidation> {

        private Supplier<Integer> m_tilesPerRow;

        @Override
        public void init(final StateProviderInitializer initializer) {
            initializer.computeBeforeOpenDialog();
            m_tilesPerRow = initializer.computeFromValueSupplier(TilesPerRowReference.class);
        }
        ...
        public MinValidation computeState(final NodeParametersInput parametersInput)
            ...
            return new MinValidation() {
                ...
                protected double getMin() {
                    return m_tilesPerRow.get();
                }

                @Override
                public String getErrorMessage() {
                    return String.format("Page size cannot be smaller than the number of tiles per row (%d).",
                        m_tilesPerRow.get());
                }
            };
        }
    ...
    public void validate() throws InvalidSettingsException {
        if (m_pageSize < m_tilesPerRow) {
            throw new InvalidSettingsException(String.format(
                "Page size (%d) cannot be smaller than the number of tiles per row (%d).", m_pageSize, m_tilesPerRow));
        }
    }
