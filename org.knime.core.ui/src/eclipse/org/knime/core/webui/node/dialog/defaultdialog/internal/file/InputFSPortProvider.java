 ...
 */
public abstract class InputFSPortProvider implements StateProvider<Optional<ConnectedFSOptions>>  {
     ...
     * instead determine the correct port based on {@link NodeParametersInput}.
     *
     * Typically used in tandem with {@link WithFileSystem}
     *
     * @return group id that identifies the port. E.g. "Destination File System Connection"
     */
    protected abstract String getGroupId();

    private static int getPortIndex(final String groupId, final NodeParametersInput context) {
    ...
    @Override
    public Optional<ConnectedFSOptions> computeState(final NodeParametersInput context) {
        var inputPortIndex = getPortIndex(getGroupId(), context);
        if (inputPortIndex == -1) {
