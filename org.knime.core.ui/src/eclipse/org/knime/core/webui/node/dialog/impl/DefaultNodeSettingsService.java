...
final class DefaultNodeSettingsService implements JsonNodeSettingsService<String> {
    ...
    private void textSettingsToNodeSettings(final ObjectNode rootNode, final SettingsType settingsType,
        ...
        if (settings.containsKey(settingsType)) {
            ...
            // TODO
            Class settingsClass = m_settingsClasses.get(settingsType);
            var settingsObj = JsonFormsDataUtil.toDefaultNodeSettings(node, settingsClass);
            // TODO if the serializer is the default one, we could actually translate the json-object directly into
            // the node-settings-object (instead of taking the detour via the 'DefaultNodeSettings'). Worth the effort?
            NodeSettingsSerializerFactory.createSerializer(settingsClass).save(settingsObj, settings.get(settingsType));
        }
    }
    ...
    private DefaultNodeSettings loadSettings(final Map<SettingsType, NodeSettingsRO> nodeSettings,
        ...
        try {
            return NodeSettingsSerializerFactory.createSerializer(settingsClass).load(nodeSettings.get(settingsType));
    ...
    private void saveDefaultSettings(final Map<SettingsType, NodeSettingsWO> settingsMap,
        ...
        if (nodeSettings != null) {
            ...
            NodeSettingsSerializerFactory.createSerializer(settingsClass).save(settingsObj, nodeSettings);
        }
    }
