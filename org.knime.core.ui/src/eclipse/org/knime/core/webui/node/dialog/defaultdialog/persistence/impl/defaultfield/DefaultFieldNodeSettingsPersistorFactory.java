...
public final class DefaultFieldNodeSettingsPersistorFactory {
    ...
    public static NodeSettingsPersistor<?> createPersistor(final LeafNode<PersistableSettings> node,
    ...
    private static <T> NodeSettingsPersistor<T> createPersistorFromImpl(final Class<T> fieldType,
        ...
        if (impl != null) {
            ...
            return (NodeSettingsPersistor<T>)createLocalTimePersistor(configKey);
        } else if (fieldType.equals(ZoneId.class)) {
            return (NodeSettingsPersistor<T>)createTimeZonePersistor(configKey);
        } else if (fieldType.equals(Interval.class)) {
            return (NodeSettingsPersistor<T>)createIntervalPersistor(configKey);
        } else if (fieldType.equals(DateInterval.class)) {
            return (NodeSettingsPersistor<T>)createDateIntervalPersistor(configKey);
        } else if (fieldType.equals(TimeInterval.class)) {
            return (NodeSettingsPersistor<T>)createTimeIntervalPersistor(configKey);
    ...
    private static NodeSettingsPersistor<LocalTime> createLocalTimePersistor(final String configKey) {
        return new LocalTimePersistor(configKey);
    }

    private static NodeSettingsPersistor<ZoneId> createTimeZonePersistor(final String configKey) {
        return new TimeZonePersistor(configKey);
    }

    private static NodeSettingsPersistor<Interval> createIntervalPersistor(final String configKey) {
        return new IntervalPersistor(configKey);
    }

    private static NodeSettingsPersistor<DateInterval> createDateIntervalPersistor(final String configKey) {
        return new DateIntervalPersistor(configKey);
    }

    private static NodeSettingsPersistor<TimeInterval> createTimeIntervalPersistor(final String configKey) {
