...
/**
 * Tile-view-specific table transfer object.
 *
...
public interface TileTable {
    ...
    /**
     * @return the content type per column. Because the tile view has no renderer selection, it uses the attached
     *         formatter of a column (i.e., the formatter attached via a format manager node) or the default renderer
     */
    ...
    /**
     * @return the per-row title values for the configured title column, or an empty array when no title column is
     *         selected.
     */
    Object[] getRowTitles();

    /**
     * @return the per-row colors for the configured color column, or an empty array when no color column is selected or
     *         available.
     */
    String[] getRowColors();
