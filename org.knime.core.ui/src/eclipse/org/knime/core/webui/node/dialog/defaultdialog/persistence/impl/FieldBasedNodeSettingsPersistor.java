 ...
 */
public class FieldBasedNodeSettingsPersistor<S extends PersistableSettings>
    implements NodeSettingsPersistorWithInferredConfigs<S> {
     ...
     */
    FieldBasedNodeSettingsPersistor(final NodeSettingsPersistorContext<S> context) {
        this(context.getPersistedObjectClass());
    }
    ...
    public ConfigMappings getConfigMappings(final S obj) {
        ...
        try {
            callForAllFields((persistor, node) -> configMappingsForFields
                .add(uncheckedGetConfigMappings(persistor, node.getFromParentValue(obj))));
        } catch (InvalidSettingsException ex) {//NOSONAR
            // because the origin of the InvalidSettingsException would be our PersistorConsumer which does not
            // throw such an exception
            throw new IllegalStateException("This catch block is not supposed to be reachable.");
    ...
    @SuppressWarnings("unchecked")
    private static <T> ConfigMappings uncheckedGetConfigMappings(final NodeSettingsPersistor<T> persistor,
        final Object value) {
    ...
    public S load(final NodeSettingsRO settings) throws InvalidSettingsException {
        ...
        callForAllFields((persistor, node) -> node.setInParentValue(loaded, persistor.load(settings)));
        return loaded;
    }
    ...
    public String[][] getNonNullPaths() {
        /**
         * One empty path, i.e. referencing everything down from this level.
         */
