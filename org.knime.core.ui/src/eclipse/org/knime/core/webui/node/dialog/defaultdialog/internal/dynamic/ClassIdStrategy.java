 ...
 *
 * History
 *   29 Aug 2025 (Paul Bärnreuther): created
 */
package org.knime.core.webui.node.dialog.defaultdialog.internal.dynamic;

/**
 * Strategy interface for handling class identification in dynamic parameters. This interface provides methods to map
 * between class identifiers and actual classes, following the composition over inheritance principle.
 *
 * @param <T> the common interface/abstract class used for these dynamic settings
 * @author Paul Bärnreuther
 */
public interface ClassIdStrategy<T> {

    /**
     * Mapping from class identifier to a java class.
     *
     * @param classIdentifier the {@link #toIdentifier} result of an implementation of T (but possibly from a different
     *            point in time).
     * @return the to be used concrete class. This class has a default constructor. Also when calling the method
     *         {@link #toIdentifier(Class)} on the returned class, the same string as the provided classIdentifier is
     *         returned.
     */
     ...
     * @return the class identifier. This identifier is stable over time for a given class.
     */
    ...
    default Class<? extends T> fromIdentifierInternal(final String classIdentifier) {
        ...
        if (!id.equals(classIdentifier)) {
            throw new IllegalStateException("The class identifier '" + classIdentifier + "' was mapped to the class '"
                ...
                + " consistent. Please check your implementation of ClassIdStrategy.");
        }
