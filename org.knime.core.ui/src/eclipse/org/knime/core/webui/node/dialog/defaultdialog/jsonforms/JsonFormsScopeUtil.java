...
public final class JsonFormsScopeUtil {
     ...
     * @param settingsType possibly null
     ...
     */
    public static ObjectNode getSchemaAtOrFail(final ObjectNode jsonSchemaWithoutProperties,
        final SettingsType settingsType, final List<String> path) {
        return traverseSchemaPath(jsonSchemaWithoutProperties, settingsType, path,
            JsonFormsScopeUtil::getObjectAtOrFail);
    }
    ...
    public static ObjectNode getOrConstructSchemaAt(final ObjectNode jsonSchemaWithoutProperties,
        final SettingsType settingsType, final List<String> path) {
        return traverseSchemaPath(jsonSchemaWithoutProperties, settingsType, path,
            JsonFormsScopeUtil::getOrConstructObjectAt);
    }

    private static ObjectNode traverseSchemaPath(final ObjectNode root, final SettingsType settingsType,
        final List<String> path, final BiFunction<ObjectNode, String, ObjectNode> navigator) {
        ObjectNode currentNode = root;
        if (settingsType != null) {
            currentNode = navigator.apply(currentNode, PROPERTIES);
            currentNode = navigator.apply(currentNode, settingsType.getConfigKey());
        }
