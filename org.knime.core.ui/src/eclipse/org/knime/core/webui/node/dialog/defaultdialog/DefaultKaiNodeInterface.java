...
public final class DefaultKaiNodeInterface implements KaiNodeInterface {
    ...
    static JsonNode getFlowVariables(final NodeContainer nc) {
        ...
        final var variableOutport = nc.getOutPort(0);

        if (variableOutport.getFlowObjectStack() != null) {
            final var flowVarStack = variableOutport.getFlowObjectStack();
            final var allFlowVariables = flowVarStack.getAllAvailableFlowVariables();
            allFlowVariables.values().stream()
                // Skip variables with GLOBAL scope (knime.workspace variable)
                .filter(flowVar -> !"GLOBAL".equalsIgnoreCase(flowVar.getScope().toString()))
                .forEach(flowVar -> {
