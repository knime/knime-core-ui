 ...
 *
 * History
 *   Jul 22, 2025 (Marc Bux, KNIME GmbH, Berlin, Germany): created
 */
...
/**
 * When using the same {@link WidgetGroup} in two different contexts with slightly different configurations, use this
 * annotation from outside to modify parts of it (e.g. changing a description, changing the choices of a dropdown or
 * adapting value updates).
 *
 * Put this annotation on a widget group (either field or class) or an array layout field in order to modify one of its
 * contained widgets. This is only useful if the widget group is used in more than one context with slightly different
 * configurations.
 *
 * As a best practice, when writing such a widget group, explain in JavaDocs what modifications are intended or define
 * an abstract implementation of {@link Modification.Modifier} that boils down to the necessary information to be
 * implemented by the different cases.
 *
 * TODO: UIEXT-2175 Re-think this API and possibly modify not by changing annotations but by a refined API that operates
 * on {@link DialogElementRendererSpec} representations of the dialog.
 */
@Retention(RUNTIME)
@Target({FIELD, TYPE})
public @interface Modification {
     ...
     *         annotations.
     */
    Class<? extends Modification.Modifier>[] value();
    ...
    /**
     * The counterpart annotation that is to be set on the fields which should be modified by a {@link Modification}.
     *
     * @author Paul Bärnreuther
     */
    @Retention(RUNTIME)
    ...
    interface WidgetGroupModifier {
        ...
        /**
         * @param reference the identifier of the widget to modify (it has to be annotated by {@link WidgetReference})
         * @return a modifier for the widget
         */
        Modification.WidgetModifier find(Class<? extends Reference> reference);
    ...
    interface WidgetModifier {
         ...
         */
        <T extends Annotation> Modification.AnnotationModifier modifyAnnotation(Class<T> annotationClass);
         ...
         */
        <T extends Annotation> Modification.AnnotationModifier addAnnotation(Class<T> annotationClass);
