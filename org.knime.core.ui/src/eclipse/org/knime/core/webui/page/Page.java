...
public class Page implements Resource, FluentNodeAPI {
    ...
    public static RequireFromFileOrString create() {
        return new RequireFromFileOrString() {
            ...
            @Override
            public RequireRelativPath fromString(final InputStreamSupplier content) {
                return relativePath -> new Page(createResource(content, relativePath, false, null), false);
            }

            @Override
            public RequireRelativPath fromString(final StringSupplier content) {
                return relativePath -> new Page(
                    createResource(() -> new ByteArrayInputStream(content.get().getBytes(StandardCharsets.UTF_8)),
                        relativePath, false, StandardCharsets.UTF_8),
                    false);
            }
        };
    }
    ...
    public interface RequireFromFileOrString {
         ...
         */
        RequireRelativPath fromString(InputStreamSupplier content);

        /**
         * @param content the page content supplier for lazy initialization
         */
        RequireRelativPath fromString(StringSupplier content);
