 ...
 * migrated from the classic UI, and
 *
 *
 *
...
public final class LegacyMultiFileSelection implements Persistable, WidgetGroup {
    ...
    static final class FileOrFolderPersistor implements NodeParametersPersistor<FileOrFolder> {
        ...
        public FileOrFolder load(final NodeSettingsRO settings) throws InvalidSettingsException {
            String mode = settings.getNodeSettings("filter_mode").getString("filter_mode", "FILE");
            if (mode.equals("FILE")) {
        ...
        public void save(final FileOrFolder param, final NodeSettingsWO settingsWO) {
            // We have two settings that save to sub-NodeSettings, but need individual
            // custom persistors, so we do a little workaround to lazily add the sub-NodeSettings
            // group here and in the other persistor's save method.
            NodeSettings settings = ((NodeSettings)settingsWO);
            NodeSettingsWO filterModeSettings;

            try {
                filterModeSettings = settings.getNodeSettings("filter_mode");
            } catch (InvalidSettingsException ex) {
                filterModeSettings = settings.addNodeSettings("filter_mode");
            }

            filterModeSettings.addString("filter_mode", param == FileOrFolder.FOLDER ? "FILES_IN_FOLDERS" : "FILE");
        }

        @Override
        public String[][] getConfigPaths() {
            return new String[][]{{"filter_mode", "filter_mode"}};
        }
    }
    ...
    static final class IncludeSubfoldersPersistor implements NodeParametersPersistor<Boolean> {
        ...
        public Boolean load(final NodeSettingsRO settings) throws InvalidSettingsException {
            String mode = settings.getNodeSettings("filter_mode").getString("filter_mode", "FILE");
            boolean includeSubfolders = settings.getNodeSettings("filter_mode").getBoolean("include_subfolders", false);
        ...
        public void save(final Boolean param, final NodeSettingsWO settingsWO) {
            ...
            // Note, that was fileOrFolder==FOLDER && includeSubfolders before
            filterModeSettings.addBoolean("include_subfolders", param);
        }

        @Override
        public String[][] getConfigPaths() {
            return new String[][]{{"filter_mode", "include_subfolders"}};
        }
    }
