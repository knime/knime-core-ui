...
# 3  In `knime-core-ui/org.knime.core.ui/js-src/packages/scripting-editor`
...
| **3** | From `org.knime.core.ui/js-src/packages/scripting-editor`: Bump version (`pnpm version prerelease --preid=local.$(git rev-parse --short HEAD)`) then publish (`pnpm publish --registry http://localhost:4873`) | Keeping the version unique prevents clashes with real npmjs.org versions. The `.npmrc` is added to the workspace root (`../../.npmrc`).                                                |
