...
describe("paths", () => {
  ...
  describe("route", () => {
    it("respects route when composing segments", () => {
      const path = "model.mySetting";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
      ...
      expect(configPaths).toStrictEqual([
        {
          configPath: "route1.route2.route3.mySetting.subConfigKey",
          deprecatedConfigPaths: [],
        },
      ]);
    });

    it("respects route in combination with config paths", () => {
      const path = "model.mySetting";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
      ...
      expect(configPaths).toStrictEqual([
        {
          configPath: "route1.route2.model_1",
          deprecatedConfigPaths: [],
        },
        {
          configPath: "route1.route2.model_2",
          deprecatedConfigPaths: [],
        },
      ]);
    });

    it("respects route in combination with config keys", () => {
      const path = "model.mySetting";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
      ...
      expect(configPaths).toStrictEqual([
        {
          configPath: "route1.route2.model_1.route3.mySetting.subConfigKey",
          deprecatedConfigPaths: [],
        },
      ]);
    });

    it("respects route in combination with deprecated config keys", () => {
      const path = "model.mySetting";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
      ...
      expect(configPaths).toStrictEqual([
        {
          configPath: "route1.route2.route3.mySetting.subConfigKey",
          deprecatedConfigPaths: ["route1.route2.deprecated"],
        },
      ]);
    });

    it("lists deprecated keys even when routing to a parent from a child", () => {
      const path = "model.mySetting";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
      ...
      expect(configPaths).toStrictEqual([
        {
          configPath: "route1.route3.mySetting.subConfigKey",
          deprecatedConfigPaths: ["route1.route2.deprecated"],
        },
      ]);
    });

    it("respects propertiesRoute", () => {
      const path = "model.mySetting";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
          model: {
            type: "object",
            propertiesRoute: ["route1", "route2"],
            properties: {
              mySetting: {
                type: "object",
                propertiesRoute: ["..", "route3"],
                properties: {
                  subConfigKey: {},
                },
              },
      ...
      expect(configPaths).toStrictEqual([
        {
          configPath: "model.route1.route2.route3.subConfigKey",
          deprecatedConfigPaths: [],
        },
      ]);
    });

    it("respects propertiesRoute in combination with propertiesConfigPaths", () => {
      const path = "model.mySetting";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
      ...
      expect(configPaths).toStrictEqual([
        {
          configPath: "modelRoute.model.route1.route2.mySetting_1",
          deprecatedConfigPaths: [],
        },
        {
          configPath: "modelRoute.model.route1.route2.mySetting_2",
          deprecatedConfigPaths: [],
        },
      ]);
    });

    it("respects propertiesRoute in combination with route", () => {
      const path = "model.mySetting";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
      ...
      expect(configPaths).toStrictEqual([
        {
          configPath: "model_1.route1.route3.mySetting.subConfigKey",
          deprecatedConfigPaths: [],
        },
      ]);
    });

    it("respects propertiesRoute in combination with route in sub config keys", () => {
      const path = "model";
      const persistSchema: PersistSchema = {
        type: "object",
        properties: {
