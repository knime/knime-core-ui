...
export const isModelSettingAndHasNodeView = (control) =>
...
export const getFlowVariablesMap = ({ rootSchema, path, schema }) => {
  if (rootSchema?.flowVariablesMap) {
    if (schema.configKeys) {
        ...
        .reduce(
          (a, b) => ({
            controllingFlowVariableAvailable:
              a?.controllingFlowVariableAvailable ||
      ...
      return rootSchema.flowVariablesMap.hasOwnProperty(path)
...
export const hasAdvancedOptions = (uischema) =>
  isKeyValuePresentInObject(uischema, "options", "isAdvanced", true);
