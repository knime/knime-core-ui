...
class DataTypeSerializationTest {
    ...
    /**
     * Test is currently failing because the deserialization does not check collection types. If we need to support this
     * we could instead of using the id use the same approach as in
     * {@link org.knime.base.node.io.filehandling.webui.reader.DataTypeStringSerializer} in order to be able to
     * reconstruct the data type without the registry, or (as far as I can see there exists only Sets and Lists) we
     * could flatmap the list of registered elements with ( *, List<*>, Set<*> ) and check if the id matches any of them.
     * We also need to decide whether collection types are supported by default in the DataType selection widget.
     */
    @Test
