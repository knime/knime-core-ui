...
class PageCacheTest {
    ...
    CreatedPage createPage(final NodeWrapper nw, final Page page) {
        return new CreatedPage(page, nw.getNodeWrapperTypeId());
    }
    ...
    void testGetPageIdAndPage() throws IOException {
        ...
        var nonStaticPageId =
            pageCache.getOrCreatePageAndReturnPageId(nodeWrapperMock, nw -> createPage(nw, nonStaticPage), true);
        ...
        pageCache.getOrCreatePageAndReturnPageId(nodeWrapperMock,
            nw -> createPage(nw, Page.builder(() -> "test2", "page2.html").build()), true);
        ...
        var staticPageId =
            pageCache.getOrCreatePageAndReturnPageId(nodeWrapperMock, nw -> createPage(nw, staticPage), true);
        assertThat(staticPageId).isEqualTo("nodeWrapperTypeId");
        assertThat(pageCache.getPage(staticPageId)).isSameAs(staticPage);
        pageCache.getOrCreatePageAndReturnPageId(nodeWrapperMock, nw -> createPage(nw, staticPageBuilder.build()),
            true);
        ...
        var reusableStaticPageId =
            pageCache.getOrCreatePageAndReturnPageId(nodeWrapperMock, nw -> createPage(nw, reusableStaticPage), true);
        ...
        pageCache.getOrCreatePageAndReturnPageId(nodeWrapperMock,
            nw -> createPage(nw, staticPageBuilder.markAsReusable("reusable_page_id").build()), true);
