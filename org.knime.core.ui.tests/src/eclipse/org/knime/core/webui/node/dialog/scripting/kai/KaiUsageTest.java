...
class KaiUsageTest {
    ...
    void testSealedInterfaceExhaustiveness() {
        // This test ensures that if a new type is added, the switch must be updated
        KaiUsage usage = new KaiUsage.Unknown();

        String result = switch (usage) {
            case KaiUsage.Unknown u -> "unknown";
            case KaiUsage.Limited l -> "limited";
            case KaiUsage.Unlimited u -> "unlimited";
            case KaiUsage.Unlicensed u -> "unlicensed";
            // If a new type is added, this will cause a compile error
        };

        assertEquals("unknown", result, "Switch expression should correctly handle Unknown type");
    }
}
