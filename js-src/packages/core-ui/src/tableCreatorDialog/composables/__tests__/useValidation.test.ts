...
const createRpcMock = () => ({
  validateCellFromStringValue: vi.fn<(dataType: string, value: string) => Promise<boolean>>(),
  validateCellsFromStringValues: vi.fn<(dataType: string, values: string[]) => Promise<boolean[]>>(),
});
...
const setup = () => {
  ...
  const validation = useValidation({ onValidityUpdate });
...
describe("useValidation", () => {
  ...
  describe("caching", () => {
    ...
    it("makes separate backend calls for different dataTypes", async () => {
      const { validation, rpcMock } = setup();
      rpcMock.validateCellFromStringValue.mockResolvedValue(true);
