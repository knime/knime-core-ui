...
globalThis.Blob = class extends OriginalBlob {
  ...
  async text(): Promise<string> {
    return this.__parts.map((p) => (typeof p === "string" ? p : "")).join("");
  }
};
...
describe("useCopyPaste", () => {
  ...
  describe("onCopySelection", () => {
    it("builds TSV and HTML, writes to clipboard, sets cursor", async () => {
      ...
      expect(htmlContent).toContain("<td>r0c0</td>");
      expect(htmlContent).toContain("<td>r0c1</td>");
      expect(htmlContent).toContain("<td>r1c0</td>");
      expect(htmlContent).toContain("<td>r1c1</td>");
    ...
    it("includes headers when withHeaders is true", async () => {
      ...
      const textBlob = await clipboardItem.getType("text/plain");
      const textContent = await textBlob.text();
      expect(textContent).toBe("Col0\tCol1\nr0c0\tr0c1\n");
    });
    ...
    it("logs error and resets cursor on clipboard write failure", async () => {
  ...
  describe("onPasteSelection", () => {
    ...
    it("logs error on clipboard read failure", async () => {
      mockClipboardRead.mockRejectedValueOnce(new Error("permission denied"));
      const consoleSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
  ...
  describe("round-trip copy/paste", () => {
    it("preserves null cells through copy and paste via HTML", async () => {
      ...
      const params = createParams({
        getCellValue: vi.fn((col: number, _row: number) =>
      ...
      mockClipboardWrite.mockImplementationOnce(async (items: any[]) => {
