...
type TableUIMethods = {
  ...
  updateCellSelection: (rect: { minX: number; minY: number; maxX: number; maxY: number }, rectId?: number | null) => void;
...
type CheckType<TMethods, TComponent extends TMethods> = TMethods;

const mockTableComponent : CheckType<TableUIMethods, InstanceType<typeof TableUI>> = {
  refocusSelection: vi.fn() ,
  updateCellSelection: vi.fn() ,
  focusHeaderCell: vi.fn(),
}
const mockColumnHeaderInput: CheckType<ColumnHeaderInputMethods, InstanceType<typeof ColumnHeaderInput>> = {
  focusColumnNameInput: vi.fn(),
};

const mockTableCreatorLayout: CheckType<TableCreatorLayoutMethods, InstanceType<typeof TableCreatorLayout>> = {
...
vi.mock("vue", async () => {
  ...
  return {
    ...
    useTemplateRef: vi.fn((name: string) => {
      if (name === testTableRef) return { value: mockTableComponent };
      if (name === testColumnHeaderInputRef) return { value: mockColumnHeaderInput };
      if (name === testTableCreatorLayoutRef) return { value: mockTableCreatorLayout };
      return { value: null };
    }),
