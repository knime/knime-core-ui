...
const mountTableCreatorDialogCore = async ({initialData, isLargeMode = true}: { initialData?: InitialData, isLargeMode?: boolean} = {}) => {
...
const getTrWithText = (wrapper: VueWrapper<any>, text: string) => {
  return wrapper.findAll("tr").filter((tr) => tr.text().includes(text))[0];
}
...
describe("TableCreatorDialogCore", () => {
  ...
  describe("cell editing", () => {
   ...
   const getHeaderCellElement = (wrapper: VueWrapper<any>, columnName: string) => {
  ...
  describe("column editing", () => {
    ...
    const focusHeaderCell = async (wrapper: VueWrapper<any>, columnName: string) => {
      getHeaderCellElement(wrapper, columnName).dispatchEvent(new FocusEvent("focus"));
      await nextTick();
    }
    ...
    it("sets focus to column name input when clicking on header cell", async () => {
     ...
     expect(columnNameInput.exists()).toBeTruthy();
      ...
      expect(document.activeElement).not.toBe(columnNameInput.element.querySelector("input"));

      headerCellElement.dispatchEvent(new MouseEvent("click"));
      await nextTick();
      expect(document.activeElement).toBe(columnNameInput.element.querySelector("input"));
    });

    it("does not show column parameters in small mode on mere focus", async () => {
      const { wrapper } = await mountTableCreatorDialogCore({ isLargeMode: false });
  ...
  describe("deletions and appends", () => {
     it("deletes column when delete button is clicked on column header", async () => {
        const { wrapper } = await mountTableCreatorDialogCore();
        const deleteButtons = wrapper.findAll('button[aria-label="Delete column"]');
     ...
     it("deletes row when delete button is clicked left of the row", async () => {
      ...
      it("appends row when append row button is clicked", async () => {
        const { wrapper } = await mountTableCreatorDialogCore();
const appendRowButton = wrapper.findAll('button').find((btn) => btn.text() === "Add row");
      ...
      it("appends column when append column button is clicked on header", async () => {
        const { wrapper } = await mountTableCreatorDialogCore();
        const appendColumnButton = wrapper.findAll('button').find((btn) => btn.text() === "Add column");
