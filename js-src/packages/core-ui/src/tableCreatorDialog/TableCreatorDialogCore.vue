...
const deleter = useDeletion({
  ...
  onDeleteRow: (rowIndex: number) => {
    ...
    dialogInitialData.data.model.numRows--;
  },
});
...
<template>
  <div class="table-creator-dialog" tabindex="-1">
    <TableCreatorLayout :is-large-mode :ref="tableCreatorLayoutRef" @refocus-table="refocusTable">
      <template #main-table>
      ...
      <template #side-panel="{close}">
        ...
        >
      <div v-if="isLargeMode && selectedColumnIndex >= 0" :tabindex="0" @focus="refocusTable" />
          <CellInput
            class="cell-input"
          ...
          <ColumnHeaderInput
            ...
            @column-name-keydown-enter="[close(), selectFirstCellInColumn(selectedColumnIndex)]"
