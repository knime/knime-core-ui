...
describe("useAutoSizes", () => {
  ...
  describe("auto row heights", () => {
    it("creates the correct auto row height options when using AUTO", () => {
      const { autoRowHeightOptions } = useAutoSizes(initialDataMock);

      expect(autoRowHeightOptions.value).toStrictEqual({
        calculate: true,
        fixedHeights: {},
        maxHeight: 120,
      });
    ...
    it("limits the minimum for the maximum auto row height to the minimum row height", () => {
      initialDataMock.settings.value.maxAutoRowHeight = 12;
      const { autoRowHeightOptions } = useAutoSizes(initialDataMock);
    ...
    it("creates the correct auto row height options when using CUSTOM", () => {
      initialDataMock.settings.value.rowHeightMode = RowHeightMode.CUSTOM;
      const { autoRowHeightOptions } = useAutoSizes(initialDataMock);
